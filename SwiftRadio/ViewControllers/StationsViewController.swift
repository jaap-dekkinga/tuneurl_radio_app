...
class StationsViewController: BaseController, Handoffable {
    ...
    // MARK: - Properties
    private let player = RadioManager.shared
...
extension StationsViewController: RadioManagerObserver {
    ...
    func radioManager(_ manager: RadioManager, metadataDidChange metadata: FRadioPlayer.Metadata?) {
        updateNowPlayingButton(station: self.manager.currentStation)
        updateHandoffUserActivity(userActivity, station: self.manager.currentStation)
    }
}
